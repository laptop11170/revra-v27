...
export default function AIProvidersPage() {
  ...
  const [activeTab, setActiveTab] = useState<"llm" | "emma" | "features">("llm");
  ...
  const [emmaProviders, setEmmaProviders] = useState<EmmaProvider[]>([]);
  ...
  const fetchProviders = useCallback(async () => {
    try {
      ...
      setEmmaProviders(json.providers?.emmaProviders || []);
    } catch (err) {
  ...
  const tabConfig = [
    ...
    { key: "emma" as const, label: "EMMA AI", icon: Phone },
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm mt-1" style={{ color: "hsl(var(--muted-foreground))" }}>Configure AI models and EMMA AI voice agent</p>
            </div>
          </div>
          <Card>
  ...
  return (
    <Shell role="superadmin">
      <div className="space-y-6">
        ...
        {activeTab === "emma" && (
          <div className="space-y-6">
            {emmaProviders.length === 0 ? (
              <Card>
                <CardContent className="p-12 flex items-center justify-center">
                  <p style={{ color: "hsl(var(--muted-foreground))" }}>No EMMA AI providers configured</p>
                </CardContent>
              </Card>
            ) : (
              emmaProviders.map((emma) => (
                <Card key={emma.id}>
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-xl flex items-center justify-center text-white font-bold text-lg" style={{ backgroundColor: `hsl(var(--${emma.colorVar}))` }}>
                          {emma.icon}
                        </div>
                        <div>
                          <div className="flex items-center gap-3">
                            <CardTitle>{emma.name}</CardTitle>
                            <Badge variant="success"><Activity size={10} className="mr-1" /> Online</Badge>
                          </div>
                          <p className="text-sm mt-1" style={{ color: "hsl(var(--muted-foreground))" }}>Endpoint: {emma.endpoint} · {emma.workspaces} workspaces</p>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <Button variant="outline" size="sm"><Settings size={14} className="mr-1" />Configure</Button>
                        <Button variant="outline" size="sm"><BarChart3 size={14} className="mr-1" />Campaigns</Button>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <div className="mb-6">
                      <h4 className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: "hsl(var(--muted-foreground))" }}>API Key</h4>
                      <div className="flex items-center justify-between p-4 rounded-xl" style={{ backgroundColor: "hsl(var(--surface-container-low))" }}>
                        <p className="text-sm font-mono" style={{ color: "hsl(var(--on-surface))" }}>
                          {showKey[emma.id] ? emma.apiKey.replace("emma-", "emma-") : "emma-••••••••••••••••"}
                        </p>
                        <div className="flex gap-2">
                          <button
                            onClick={() => toggleShow(emma.id.toString())}
                            className="p-1.5 rounded-md transition-colors"
                            style={{ color: "hsl(var(--muted-foreground))" }}
                            onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = "hsl(var(--surface-container-high))")}
                            onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = "transparent")}
                          >
                            {showKey[emma.id] ? <EyeOff size={14} /> : <Eye size={14} />}
                          </button>
                          <Button variant="outline" size="sm" onClick={() => handleCopyApiKey(emma.apiKey)}>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                      <div className="p-4 rounded-xl text-center" style={{ backgroundColor: "hsl(var(--primary)_/_0.1)" }}>
                        <p className="text-3xl font-bold" style={{ color: "hsl(var(--primary))" }}>{emma.stats.activeCalls}</p>
                      ...
                      <div className="p-4 rounded-xl text-center" style={{ backgroundColor: "hsl(var(--info)_/_0.1)" }}>
                        <p className="text-3xl font-bold" style={{ color: "hsl(var(--info))" }}>{emma.stats.queuedCalls}</p>
                        <p className="text-xs mt-1" style={{ color: "hsl(var(--info))", opacity: 0.8 }}>Queued</p>
                      </div>
                      <div className="p-4 rounded-xl text-center" style={{ backgroundColor: "hsl(var(--success)_/_0.1)" }}>
                        <p className="text-3xl font-bold" style={{ color: "hsl(var(--success))" }}>{emma.stats.completedToday.toLocaleString()}</p>
                        <p className="text-xs mt-1" style={{ color: "hsl(var(--success))", opacity: 0.8 }}>Completed Today</p>
                      </div>
                      <div className="p-4 rounded-xl text-center" style={{ backgroundColor: "hsl(var(--warning)_/_0.1)" }}>
                        <p className="text-3xl font-bold" style={{ color: "hsl(var(--warning))" }}>{emma.stats.avgDuration}</p>
                        <p className="text-xs mt-1" style={{ color: "hsl(var(--warning))", opacity: 0.8 }}>Avg Duration</p>
                      </div>
                      <div className="p-4 rounded-xl text-center" style={{ backgroundColor: "hsl(var(--surface-container-high))" }}>
                        <p className="text-3xl font-bold" style={{ color: "hsl(var(--on-surface-variant))" }}>{emma.stats.successRate}</p>
                        <p className="text-xs mt-1" style={{ color: "hsl(var(--muted-foreground))" }}>Success Rate</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
