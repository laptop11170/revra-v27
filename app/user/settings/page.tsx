    ...
    <Shell role="user">
      <div style={{ padding: "32px 40px" }}>
        ...
        <div style={{ display: "flex", gap: 0 }}>
          ...
          >
            {sections.map((s) => {
              const Icon = s.id === "integrations" ? PlugIcon : s.id === "notifications" ? Bell : s.icon;
              return (
                <button
                  key={s.id}
          ...
          <div style={{ flex: 1, maxWidth: 560 }}>
            {activeSection === "profile" && (
              <div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 24 }}>
                  ...
                  <button className="btn-ghost" style={{ padding: "6px 14px", fontSize: 12 }}>Change Photo</button>
                </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                    <div>
                      ...
                      <input className="input" defaultValue="Sales" />
                    </div>
                    <div>
                      <label className="input-label">Last Name</label>
                      <input className="input" defaultValue="Team" />
                    </div>
                  </div>
                  <div>
                    <label className="input-label">Email</label>
                    <input className="input" type="email" defaultValue="sales@yourcompany.com" />
                  </div>
                  <div>
            ...
            {activeSection === "integrations" && (
              <div>
                <h3 style={{ fontSize: 15, fontWeight: 600, color: "var(--ink)", marginBottom: 20 }}>Integrations</h3>
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {[
                    { name: "Google Calendar", desc: "Sync appointments with Google Calendar", color: "#ea4335", init: "G", connected: true },
                    { name: "Twilio", desc: "SMS and voice calls", color: "#f22f46", init: "T", connected: true },
                    { name: "SendGrid", desc: "Transactional and marketing emails", color: "#1A82E2", init: "S", connected: false },
                    { name: "LoopMessages", desc: "iMessage, SMS, RCS, WhatsApp", color: "#6366F1", init: "L", connected: false },
                  ].map((it) => (
                    <div
                      key={it.name}
                      style={{
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 11.5, color: "var(--ink-mute)" }}>{it.desc}</div>
                        </div>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <button className="btn-icon p-2">
                          {it.connected ? <RefreshCw size={14} /> : <ExternalLink size={14} />}
                        </button>
                      </div>
                    </div>
                  ))}
...
function PlugIcon({ size }: { size?: number }) {
  return (
    <svg width={size || 15} height={size || 15} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <path d="M18 8H6a1 1 0 0 0-1 1v3a6 6 0 0 0 12 0v-3a1 1 0 0 0-1-1Z" />
    </svg>
  );
}