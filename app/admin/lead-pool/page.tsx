...
export default function LeadPoolPage() {
  ...
  const handleAddLead = (data: any) => {
    ...
    setPoolLeads((p) => [newLead, ...p]);
    addToast({ type: "success", title: "Lead Created", description: `${data.firstName} ${data.lastName} added to pool` });
  };

  const handleCSVImport = (data: { leads: any[] }) => {
    const newLeads = data.leads.map((l, i) => ({
      id: `l${Date.now()}-${i}`,
      first_name: l.firstName || "",
      last_name: l.lastName || "",
      phone_primary: l.phonePrimary || "",
      email: l.email || "",
      coverage_type: l.coverageType || "ACA (Individual & Family)",
      lead_source: l.leadSource || "CSV Import",
      score: 50,
      pipeline_stage: "New Lead",
      assigned_agent_name: "Unassigned",
      status: "warm",
    ...
    setPoolLeads((p) => [...newLeads, ...p]);
    addToast({ type: "success", title: "CSV Imported", description: `${newLeads.length} leads added to pool` });
  };
  ...
  return (
    <Shell role="admin">
      ...
      <AddLeadModal open={showAddLead} onClose={() => setShowAddLead(false)} onSave={handleAddLead} />
